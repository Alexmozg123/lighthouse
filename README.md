...
Конвейер живёт в одном `Flow` на `Dispatchers.IO`. UI подписывается на `StateFlow` и перерисовывает превью с оверлеем поверх кадра. На каждый кадр `SpotlightController` пересчитывает нормализованные координаты центра выбранного лица в 16-bit pan/tilt и отправляет Art-Net пакет по unicast на каждую зарегистрированную голову.
