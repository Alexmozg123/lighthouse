...
/**
 * EN: A single face detection result from [tracker.adapter.camera.YuNetDetector].
 * All coordinates are in **original frame pixels** — UI code must apply the letterbox
 ...
 *
 * RU: Результат обнаружения одного лица от [tracker.adapter.camera.YuNetDetector].
 * Все координаты — в **пикселях исходного кадра**; UI-код обязан применить
 * letterbox-трансформацию перед отрисовкой.
 *
 * @param boxX       left edge of bounding box / левый край bbox
 * @param boxY       top edge of bounding box / верхний край bbox
 * @param boxW       bounding box width / ширина bbox
 * @param boxH       bounding box height / высота bbox
 ...
 * @param rightMouth right mouth corner / правый угол рта
 * @param leftMouth  left mouth corner / левый угол рта
 * @param score      detection confidence [0, 1] / уверенность детектора [0, 1]
 */
