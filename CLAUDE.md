...
План разработки лежит в `/Users/abortsov/.claude/plans/fizzy-giggling-wall.md` — план статичный, актуальная картина в этом файле.
...
```
gradle/libs.versions.toml         # все версии и координаты зависимостей
build.gradle.kts                  # plugins через alias(libs.plugins.…)
settings.gradle.kts
gradle.properties                 # shared: kotlin.code.style, jvmargs (commit-able)
src/main/kotlin/tracker/
    ...
    app/TrackingPipeline.kt       # единый Flow<DetectedFrame>: capture → detect → BufferedImage → ImageBitmap
